...
class Gracenote
{
    ...
    public function register($clientID = null)
    {
		...
		$res = $client->request($this->_apiURL,"POST");

		if((int)$res->getStatusCode()>=400)
		{
			log_message("Woops, something went wrong with the register HTTP request with Gracenote.");
			return false; //If something wrong happened like 4XX or 5XX errors return false
		}
        // Cache it locally then return to user.
        $this->_userID = (string)$res->RESPONSE->USER;
        return $this->_userID;
    }
/**
